...
from diot import Diot, SnakeDiot, OrderedDiot
...
def test_conversion_box():
    ...
    assert list(bx.accessible_keys()) == [
        "_3",
        "_not",
        "_box_config",
        "CamelCase",
...
def test_to_json(tmp_path):
    ...
    a = Diot(**test_dict)
    assert json.loads(a.to_json(indent=0)) == {
        key: val for key, val in test_dict.items() if key != "diot_nest"
    }
